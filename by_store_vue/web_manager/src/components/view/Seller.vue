...
<script>
    export default {
        ...
        methods: {
            changeStatus(){
                let vm = this;
                vm.axios.post(vm.API.SELLER_FINDBYSTATUS_URL+"?status="+vm.radio).then(res => {
                    vm.sellerList = res.data;
                }).catch(reason => {
            ...
            pageHandler(page) {
                let vm = this;
                this.page = page;
                vm.axios.post(vm.API.SELLER_FINDPAGE_URL + "?page=" + vm.page + "&rows=" + vm.pageSize, vm.entity).then(res => {
